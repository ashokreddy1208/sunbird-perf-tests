...
echo "FileRequest = " ${FileRequest}
echo "sendTelemetryApi = " ${sendTelemetryApi}
...
sed "s#CSV_FILE#${FileRequest}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH

sed "s#PATH_PREFIX#${sendTelemetryApi}#g" $JMX_FILE_PATH > jmx.tmp
mv jmx.tmp $JMX_FILE_PATH
